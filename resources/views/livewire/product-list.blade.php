    <div class="w-full font-medium max-md:max-w-full">
            ...
            @if (auth()->user()->role === 'warehouse')
                ...
                <div class="relative inline-block text-left">
                    <button id="filterButton" type="button"
                        class="btn-hover bg-slate-100 px-4 py-3 rounded-xl flex items-center gap-2 text-slate-600"
        ...
        <div class="flex flex-col pb-8 my-10 w-full text-xl rounded-2xl bg-slate-100 max-md:max-w-full overflow-auto">
            <table class="min-w-[800px] w-max gap-2 border-collapse" id="sortable-product">
                <thead>
                        ...
                        <div class="flex gap-5 items-center self-stretch">
                            @foreach ($products->getUrlRange(1, $products->lastPage()) as $page => $url)
                                @if ($page == $products->currentPage())
                                    <span class="self-stretch px-3 py-1 text-white rounded-md bg-slate-600 text-center pulse-effect"
                                        aria-current="page">
                                        {{ $page }}
                                    </span>
                                @else
                                    <a href="{{ $url }}" class="btn-hover" aria-label="Go to page {{ $page }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
