...
@section('content')
    <section class="px-5 pt-12 pb-12 w-full">
        ...
        <form class="max-w-full" action="{{ route('warehouse.suppliers.update', $supplier->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-14">
                <input type="text" name="name" placeholder="Nama supplier"
                    class="pb-2.5 w-full text-xl border-b border-solid border-b-gray-400 text-zinc-400 bg-transparent" value="{{ $supplier->name }}"/>
            </div>

            <div class="mb-14">
                <input type="text" placeholder="Alamat" name="address"
                    class="pb-2.5 w-full text-xl border-b border-solid border-b-gray-400 text-zinc-400 bg-transparent" value="{{ $supplier->address }}"/>
            </div>

            <div class="flex gap-20 mb-14 max-sm:flex-col max-sm:gap-5">
                <div class="flex-1">
                    <input type="text" placeholder="Latitude" name="latitude"
                        class="pb-2.5 w-full text-xl border-b border-solid border-b-gray-400 text-zinc-400 bg-transparent" value="{{ $supplier->latitude }}"/>
                </div>
                <div class="flex-1">
                    <input type="text" placeholder="Longitude" name="longitude"
                        class="pb-2.5 w-full text-xl border-b border-solid border-b-gray-400 text-zinc-400 bg-transparent" value="{{ $supplier->longitude }}"/>
                </div>
            </div>

            <div class="mb-14">
                <input type="text" placeholder="Nomor HP" name="phone"
                    class="pb-2.5 w-full text-xl border-b border-solid border-b-gray-400 text-zinc-400 bg-transparent" value="{{ $supplier->phone }}"/>
            </div>
